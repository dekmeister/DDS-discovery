...
- Do not refactor across labs. Labs may duplicate each other freely.
...
src/vtslab/          the actor processes
...
- Pin `cyclonedds` in `requirements.txt`. The binding trails Python
  ...
  manylinux wheel, and that wheel bundles libddsc, the IDL compiler and the
  `cyclonedds` CLI, so there is no separate Cyclone DDS install on this host.
